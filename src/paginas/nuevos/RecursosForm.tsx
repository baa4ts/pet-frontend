...
const PreviewArchivo = ({ ap }: { ap: ArchivoPreview }) => {
    ...
    if (tipo.startsWith("image/") && ap.preview) {
        return <img src={ap.preview} className="w-full h-full object-cover" alt={ap.archivo.name} />
    }

    if (tipo.startsWith("video/") && ap.preview) {
        return <video src={ap.preview} className="w-full h-full object-cover" muted controls />
    }
    ...
    if (tipo === "application/pdf") {
        return (
            <div className="w-full h-full flex flex-col items-center justify-center gap-2 bg-muted rounded-md">
                ...
                <span className="text-xs text-muted-foreground truncate px-3 max-w-full">{ap.archivo.name}</span>
            </div>
        )
    }
...
const RecursosForm = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setEnviando(true)
        const toastId = toast.loading(`Subiendo "${ap.archivo.name}"...`, { position: "top-center" })

        const ok = await actionNuevoRecurso(ap.archivo)

        if (ok) {
            toast.success(`"${ap.archivo.name}" subido correctamente`, { id: toastId, position: "top-center" })
            setTimeout(() => navigate("/dashboard/recursos", { replace: true }), 500)
        } else {
            toast.error(`Error al subir "${ap.archivo.name}"`, { id: toastId, position: "top-center" })
            setEnviando(false)
        }
    }
